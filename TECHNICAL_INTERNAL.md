...
**Hosting path:** **Supabase Postgres + Vercel** with txn **`DATABASE_URL`**, **Session `:5432` pool `DIRECT_URL`** (or `db.<ref>` `:5432` when IPv6 is available), **`AUTH_SECRET`**. Builds validate both datasource URLs; shipped traffic uses **`DATABASE_URL`** only (`directUrl` is Prisma DDL/migrations).
