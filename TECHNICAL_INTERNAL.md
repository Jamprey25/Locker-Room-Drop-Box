...
**Persistence:** Prisma datasource is **`postgresql`**: **`DATABASE_URL`** drives the Prisma Client (**Supabase txn pool `:6543` + `pgbouncer=true`** for serverless), and **`DIRECT_URL`** maps to Prisma **`directUrl`** for **DDL** (`db push`, migrate introspection)—avoid hanging PgBouncer transaction hosts. [`scripts/postinstall-prisma.mjs`](scripts/postinstall-prisma.mjs) injects sentinel values for **`DATABASE_URL` + `DIRECT_URL`** during clean `npm install`. Runtime [`bootstrap-database-url.ts`](src/lib/bootstrap-database-url.ts) never invents credentials; Next loads **`.env` / `.env.local`**. Historical **SQLite `prisma/dev.db`** clones are orphaned from the canonical schema unless manually exported.
...
**Hosting path:** **Supabase Postgres + Vercel** with pooled **`DATABASE_URL`**, DDL-safe **`DIRECT_URL`**, and **`AUTH_SECRET`**; **`npm run db:push`** from a laptop targeting **`:5432` direct** evolves schema before traffic hits pooled connections.
